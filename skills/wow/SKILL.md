...
Every phase: Implement -> `/flow-code-review` -> `/flow-quality-check` -> `/flow-commit` -> `/flow-self-improvement` -> `/flow-report`. This is fully automatic after implementation — no pauses, no "ready for review?" questions. The user's plan approval is pre-approval for the entire pipeline.
