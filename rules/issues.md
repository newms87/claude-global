...
| Tool | Args | Purpose |
|---|---|---|
| `danx_issue_save` | `{id}` | Validate + save the YAML at `<repo>/.danxbot/issues/open/<id>.yml`. Tracker push is detached (worker poll). Returns `{saved: true}` or `{saved: false, errors[]}`. Call after every meaningful Edit. |
| `danx_issue_create` | `{filename}` | Take a draft YAML at `<repo>/.danxbot/issues/open/<filename>.yml` (with `id: ""`), assign next `ISS-N`, rename, sync to tracker. Returns `{created: true, id}` or `{created: false, errors[]}`. |
