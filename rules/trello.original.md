...
**Epic mechanics:** Add Epic label to parent, then IMMEDIATELY create all phase cards (`Epic Title > Phase N: Description`), each with own description/AC/Progress/label. The planning agent has full context — capture it into the phase cards NOW, not later. Deferring phase-card creation to "when work starts" is wrong: the next agent has less context than the planner, and recreating the design from scratch wastes the planner's understanding.

**Where phase cards go:** Same list as the parent epic at time of creation. If the epic is in Review awaiting human approval, the phase cards also go in Review. If the epic is already in In Progress when it's split, the phase cards go in In Progress. Phase cards move with the epic through the lifecycle — when the human approves the epic (moves to ToDo or In Progress), they approve the whole epic including its phases.

**After completing each phase card:** Move the phase card to Done (position: "top") immediately after the phase commit. Do not wait until the epic is complete — each phase card has its own lifecycle. Then update the epic — see `/flow-commit` for the full phase completion checklist (AC items, handoff comment, epic review). Then look for the next phase card (check In Progress first, then ToDo). When ALL phase cards are Done, move the epic to Done with a retro comment summarizing all phases.

**CRITICAL: Update the next phase card before ending the session.** The next phase will be picked up by a fresh agent with zero context from this session. After completing a phase, post a "Notes from Phase N" comment on the next phase card with anything that could cause the next agent to waste time or make mistakes: discovered constraints, timing gotchas, reusable helpers and their paths, cost/budget observations, dependencies between phases, corrections to the card description. Assume the next agent reads ONLY the card description and comments — not the epic handoff, not conversation history, not git log.
