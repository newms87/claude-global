...
**CRITICAL: A phase with no code change still runs the pipeline.** Verification-only phases, plan-only phases, and research phases are NOT exempt. You still run `/flow-report` at phase end and `/flow-finish` at session end. The code-oriented steps (`/flow-code-review`, `/flow-quality-check`, `/flow-commit`) are no-ops on an empty diff — you still invoke them and they exit cleanly with "nothing to review / nothing to commit." The pipeline is not conditional on code being changed; it is the structure for phase and session closure.

**`/flow-finish` is NEVER optional.** Regardless of whether the session produced commits, plans, or only investigation, every session ends with `/flow-finish`. It captures unwritten knowledge and formalizes Action Items that would otherwise be lost when the context window is destroyed. Skipping `/flow-finish` because "I already filed the action items manually" or "there was no code change" is the classic end-of-session shortcut. Do not take it.

**The rationalization to watch for:** "No code change, so no pipeline." This is wrong. The pipeline is the structure, not the work — it is what ensures closure happens. If you find yourself skipping steps because the phase was light, STOP and run them anyway.
